...
def settings_tab():
    ...
    num_inference_steps = st.slider("Number of inference steps", value=50, min_value=1, max_value=100,
                                    help="More steps usually lead to a higher quality of image and slower inference.")

    number_of_pictures = st.slider("Number of pictures", value=1, min_value=1, max_value=10,
...
def main():
    ...
    with st.sidebar:
        st.header("Previous image:")
        output_image = get_image(OUTPUT_IMG)
        if output_image:
            st.image(output_image)
        else:
            st.markdown("No output created yet.")
    st.write("NOTE: if you see a completely black image after generation, try increasing the \"Number inference steps\" "
             "in the settings")
